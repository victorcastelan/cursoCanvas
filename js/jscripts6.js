var js = {
	...
	initApp : function() {
		...
		js._ball.x = js.lienzo.width/2;
		js._ball.y = js.lienzo.height/2;
		
		window.addEventListener('keydown', js.pulsaTecla);
		window.addEventListener('keyup', js.sueltaTecla);
		
		js.drawFrame();
	},
	
	sueltaTecla : function(event) {
		js._ax = js._ay = 0;
	},
	
	pulsaTecla : function(event) {
		var codesX = {
			37 : -0.1, //izq
			39 : 0.1 //der
		},
		...
		if (codesX.hasOwnProperty(event.keyCode))
			js._ax = codesX[event.keyCode];
			
		if (codesY.hasOwnProperty(event.keyCode))
			js._ay = codesY[event.keyCode];
	},
